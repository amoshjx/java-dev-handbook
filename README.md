...
面向**高级 Java 开发者**的离线 HTML 多页速查站点：25 章，按软件工程生命周期编排。
...
- `19-testing-quality.html` ~ `25-agent-development.html`：测试·性能·交付·AI（一级章）
- `glossary.html`：术语字典（统一口径）
- `assets/`：样式与前端脚本

侧边栏为 **2 级目录**：`01`–`04` 与 `19`–`25` 为一级项；`05`–`18` 归入 **开发实现** 分组（缩进子项）。章节 prev/next 线性导航仍为 03 → 04 → … → 18 → 19 → … → 25。
